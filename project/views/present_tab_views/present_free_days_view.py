...
class PresentFreeDaysView(QWidget):
    ...
    def _init_ui(self):
        ...
        self.table.setColumnCount(len(strs.PRESENT_FREE_DAYS_HDR))
        self.table.setHorizontalHeaderLabels(strs.PRESENT_FREE_DAYS_HDR)
        ...
        update_button.clicked.connect(self._update_free_days)

        delete_button = QPushButton(self)
        delete_button.setText(strs.DELETE_BTN)
        delete_button.clicked.connect(self._delete_free_days)

        print_button = QPushButton(self)
        print_button.setText(strs.PRINT_BTN)
        print_button.clicked.connect(self._print_free_days)
        ...
        if row_index is not None:
            ...
            if dialog.exec():
                ...
                if response == Responses.success:
                    QMessageBox.information(self, strs.PRESENT_MSG, strs.FREE_DAYS_UPD_SUCC_MSG)
                    self._change_label()
                else:
                    QMessageBox.warning(self, strs.PRESENT_MSG, strs.FREE_DAYS_UPD_FAIL_MSG)

    def _delete_free_days(self):
        row_index = self._check_selection()
        ...
        if row_index is not None:
            ...
            if dialog.exec():
                ...
                if response == Responses.success:
                    QMessageBox.information(self, strs.PRESENT_MSG, strs.FREE_DAYS_DEL_SUCC_MSG)
                    self._change_label()
                else:
                    QMessageBox.warning(self, strs.PRESENT_MSG, strs.FREE_DAYS_DEL_FAIL_MSG)

    def _print_free_days(self):
